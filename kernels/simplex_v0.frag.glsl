vec2 VertexColor(vec2 cell)
{
    mat2 colormat = mat2(.3427, .25664,
                         .1883, .283);
    return fract(colormat*cell) * 2.0 - vec2(1.0);
}

float VertexIntensity(vec2 cell)
{
...
void imageMain(inout vec4 frag_color, vec2 frag_coord)
{
    ...
    vec3 weights = max(vec3(0.0), vec3(0.5) - distances*distances);
    weights = weights * weights * weights * weights;
    //weights = distances;

    vec2 color = vec2(
        VertexIntensity(neighbours[0]) * weights.x +
        VertexIntensity(neighbours[1]) * weights.y +
        VertexIntensity(neighbours[2]) * weights.z) * 50.0;
