...
float scene(vec3 p)
{
	return smooth_min(GDF_ico(p, 1.0, 32.0),
                      GDF_ico(rotateX(3.1415926536 * 0.25) * (p - vec3(sin(iTime), cos(iTime), 0.0)), 1.0, 32.0),
                      2.0);
}
...
void imageMain(inout vec4 frag_color, vec2 frag_coord)
{
	...
	vec3 position = vec3(sin(iTime * 0.2), cos(iTime * 0.2), -5.0);
