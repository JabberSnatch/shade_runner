...
{
    //return 8.5 * (1.0-length(p));
    float density = 0.0;
    float max_i = exp2(octave);
    float bd = 8.5;// + exp2(abs(sin(iTime) * 6.0));// * 10.0;
    float expo = 1.0;
    float scale = 4.5;
    float offset = +0.115;// + cos(iTime*0.2) * 0.2;
    float of = 1.2;
...
void imageMain(inout vec4 frag_color, vec2 frag_coord)
{
    ...
    vec3 Li_sky = vec3(0.1, 0.55, 0.85) * 0.8;// * 0.4 + vec3(0.0, 0.1, 0.0);
